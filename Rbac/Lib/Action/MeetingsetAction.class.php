<?php
class MeetingsetAction extends CommonAction {			
	...
	//执行add
	public function name_add1(){
    	$data['name']=$_REQUEST['name'];
    	$data['type']="1";
		$res = M("meetingset")->where($data)->find();
		if($res || empty($_REQUEST['name'])){
			return $this->error('添加失败,分类不能为空或已经存在!');
		}
    	$data['ctime']=time();
    	$data['c_name']=$_SESSION['loginUserName'];
    	$res=M('meetingset')->add($data);
    	if($res){
    		$this->success('添加成功.');
    	}else{
    		$this->error('添加失败,请重试!');
	...
	//编辑工程类
	public function classify_edit1(){
		$id=$_REQUEST['id'];
		$info=M('meetingset')->where(array('id'=>$id))->find();
		$this->assign('name',$info["name"]);
		$this->assign('id',$id);
		$this->display();
	}

	public function name_edit1(){
		$map['name'] = $_REQUEST['name'];
		$map['type'] = 1;
		$map['id'] = array("neq",$_REQUEST['id']);
		$res = M("meetingset")->where($map)->find();
		if($res || empty($_REQUEST['name'])){
			return $this->error('添加失败,分类不能为空或已经存在!');
		}
		$id=$_REQUEST[id];
		...
		$data['ctime']=time();
		$data['c_name']=$_SESSION['loginUserName'];
		$res=M('meetingset')->where(array('id'=>$id))->save($data);
		if($res){
			$this->success('修改成功.');
		}else{
			$this->error('修改失败,请重试!');
	...
	//执行add
	public function name_add2(){
    	$data['type']="2";
    	$data['name']=$_REQUEST['name'];
		$res = M("meetingset")->where($data)->find();
		if($res || empty($_REQUEST['name'])){
			return $this->error('添加失败,属性不能为空或已经存在!');
		}
    	$data['ctime']=time();
    	$data['c_name']=$_SESSION['loginUserName'];
    	$res=M('meetingset')->add($data);
	...
	//编辑工程类
	public function classify_edit2(){
		$id=$_REQUEST['id'];
		$info=M('meetingset')->where(array('id'=>$id))->find();
		$this->assign('name',$info["name"]);
		$this->assign('id',$id);
	...
	//执行add
	public function name_add3(){
    	$data['type']="3";
    	$data['name']=$_REQUEST['name'];
		$res = M("meetingset")->where($data)->find();
		if($res || empty($_REQUEST['name'])){
			return $this->error('添加失败,类别不能为空或已经存在!');
		}
    	$data['ctime']=time();
    	$data['c_name']=$_SESSION['loginUserName'];
    	$res=M('meetingset')->add($data);
	...
	//编辑工程类
	public function classify_edit3(){
		$id=$_REQUEST['id'];
		$info=M('meetingset')->where(array('id'=>$id))->find();
		$this->assign('name',$info["name"]);
		$this->assign('id',$id);
	...
	//执行add
	public function name_add4(){
    	$data['name']=$_REQUEST['name'];
		$data['type']="4";
		$res = M("meetingset")->where($data)->find();
		if($res || empty($_REQUEST['name'])){
			return $this->error('添加失败,模版名称不能为空或已经存在!');
		}
    	$data['agenda']=$_REQUEST['agenda'];
    	$data['attendee']=$_REQUEST['attendee'];
    	$data['materials']=$_REQUEST['materials'];
    	$data['ctime']=time();
    	$data['c_name']=$_SESSION['loginUserName'];
    	$res=M('meetingset')->add($data);
    	if($res){
	...
	//编辑工程类
	public function classify_edit4(){
		$dept = M("dept")->select();
		foreach($dept as $k => $v){
			$dept[$k]['user'] = M("user")->where("department=".$v['id'])->select();
		}
	...
	//执行add
	public function name_add5(){
    	$data['name']=$_REQUEST['name'];
		$data['type']="5";
		$res = M("meetingset")->where($data)->find();
		if($res || empty($_REQUEST['name'])){
			return $this->error('添加失败,消息设置不能为空或已经存在!');
		}
    	$data['ctime']=time();
    	$data['c_name']=$_SESSION['loginUserName'];
    	$res=M('meetingset')->add($data);
	...
	//编辑工程类
	public function classify_edit5(){
		$id=$_REQUEST['id'];
		$info=M('meetingset')->where('id='.$id)->find();
		$this->assign('info',$info);
		$this->assign('id',$id);
		$this->display();
	}

	public function name_edit5(){
		$map['name'] = $_REQUEST['name'];
		$map['type'] = 5;
		$map['id'] = array("neq",$_REQUEST['id']);
		$res = M("meetingset")->where($map)->find();
    ...
    public function classify_del(){
    	$id=$_REQUEST['id'];
    	$res=M('meetingset')->where(array('id'=>$id))->delete();
    	if($res){
    		echo 1;
    	}else{
    		echo 2;
