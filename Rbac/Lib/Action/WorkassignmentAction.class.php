<?php
class WorkassignmentAction extends CommonAction {
	//过滤查询字段
	...
	function insert() {
    	...
    	if($list !== false){
			//添加任务通知
			$date=date('Y-m-d H:i');
			$data['content']=$_SESSION['loginUserName']."于".$date."委派给您一项高层交办任务，任务名称：".$_REQUEST['title']."，请及时处理。";
			$data['href'] ="index.php?s=/Workassignment/index/moduletitle/高层交办/";
			$data['taskid'] =time();
    ...
    function update() {
    	...
    	if (false !== $list) {
			$date=date('Y-m-d H:i');
			$emaildata['content'] =$_SESSION['loginUserName']."于".$date."更新了一项高层交办任务，任务名称：".$_REQUEST['title']."，您是查看人，请关注。";
			$emaildata['status']=1;
			$emaildata['sender']="系统通知";
			$emaildata['create_time']=time();
			$emaildata['title'] =$_SESSION['loginUserName']."于".$date."更新了一项高层交办任务，任务名称：".$_REQUEST['title']."，您是查看人，请关注。";
			$scheduleuser=explode(',',$_REQUEST['current']);
			foreach ($scheduleuser as $key=>$value){
				if($value!=null){
	...
	public function getexcel(){
		...
		for($k=0;$k<$count;$k++){
			...
			$mapforProject["type"] = "3";
			$mapforProject["create_time"] = time();
			$x=M("workassignment")->add($mapforProject);
			//添加任务通知
			$date=date('Y-m-d H:i');
			$datainfo['content']=$_SESSION['loginUserName']."于".$date."委派给您一项高层交办任务，任务名称：".$data[$k]['B']."，请及时处理。";
			$datainfo['href'] ="index.php?s=/Workassignment/index/moduletitle/高层交办/";
