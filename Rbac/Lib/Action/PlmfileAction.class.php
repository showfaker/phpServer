...
class PlmfileAction extends CommonAction {	
	...
	public function index() {
		...
		if(($_REQUEST['tab']==5)||(empty($_REQUEST['tab'])||($_REQUEST['tab']==9))){
			...
			$con['projecttype'] = array("neq","承揽项目");
			...
			{
				$con[sendtask_time]=array("exp","is not null");
			}
			...
			if(!empty($_REQUEST['plmid']))
			{
				...
				$map[worktype]=array("exp","is not null");
				...
				$folders=M("Classifyfolder")->where(array('status'=>0,'id'=>array("elt",91)))->select();
	...
	public function draft()
	{
		...
		$mapforprojectforstep['projecttype'] = array("neq","承揽项目");
		$mapforprojectforstep['design_status']=array("not in","取消,暂停中,暂存");
		//$mapforprojectforstep[activity]=array("exp","is not null");
		$mapforprojectforstep['_complex'] = $this->find5level($_SESSION[position],$mapforprojectforstep);
		
		if(($_SESSION["role"]=="监理单位")||($_SESSION["role"]=="施工单位")||($_SESSION["role"]=="设计单位")||($_SESSION["role"]=="投标单位"))
		{
			$mapforprojectforstep[sendtask_time]=array("exp","is not null");
		}
	...
	public function insert(){
		...
		$this->redirect('index','tab=5&plmid='.$_POST[plmid].'/classify/'.$_REQUEST[classify]."/");
	}
