<?php
class Xmtj2Action extends CommonAction {		
	...
	public function index() {
		...
		$this->assign('capacity1',$capacity1);
		$this->assign('capacity2',$capacity2);
		$this->assign('capacity3',$capacity3);
		$this->assign('capacity4',$capacity4);
		$this->assign('capacity5',$capacity5);
		$this->assign('capacity6',$capacity6);
		$this->assign('capacity7',$capacity7);
		$this->assign('capacity8',$capacity8);
		
		$this->assign('count9',$count9);
		$this->assign('count10',$count10);
		$this->assign('count11',$count11);
		
		$this->assign('count9_1',$count9_1);
		$this->assign('count10_1',$count10_1);
		$this->assign('count11_1',$count11_1);
		...
		if($_REQUEST["toexcel"]=="1")
		{
			...
			*/
			//$filename = $file;
			$excelname=$month."月度报告";
			$filename = $excelname."_".date("Y-m-d-H-i-s");
	...
	function autoDownloadList() {
		//取得满足条件的记录数
		$model = M('autodownload');
        ...
        if ($count > 0) {
            ...
            $voList = $model->order('create_time desc')->limit($p->firstRow . ',' . $p->listRows)->select();
			
			
			foreach ($voList as $key => $val1) {
				$voList[$key]['create_time'] = date('Y-m-d H:i:s',$val1['create_time']);
			}
            //分页显示
            $page = $p->show();
	...
	public function toexcel()
	{
		...
		$th_array=array('项目编号','项目名称','项目类型','合作模式','开始日期','对接日期','省','市','区','项目地址','纬度','经度','开发人员','创建日期','项目状态');
