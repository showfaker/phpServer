<?php
class SecondsuspendAction extends CommonAction {
	//过滤查询字段
	...
	public function index() {
		...
		*/
		...
		{
			$map['_complex'] = $this->find5level_1($_SESSION[position]);
		}
	...
	public function resume() {
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
                $condition = array($pk => array('in', explode(',', $id)));
				$oldinfo=M("Project")->where($condition)->find();
                $model->where($condition)->setField("design_status",$oldinfo["design_status_old"]);
				if(!empty($oldinfo["suspend_scheduleid"]))
				{
					
					$schedulemap[taskid]=$oldinfo[id];
					$schedulemap[id]=$oldinfo["suspend_scheduleid"];
					M("Schedule")->where($schedulemap)->setField("status",1);
					
					$model->where($condition)->setField("suspend_scheduleid","");
				}
				$date=date('Y-m-d H:i');
				$handlehistory=$oldinfo['handlehistory'].$_SESSION['loginUserName']."恢复暂停项目"."</br>时间：".$date."</br>------------------</br>";
				$model->where($condition)->setField("handlehistory",$handlehistory);
				
				
				
				$time=time();
				$timedistance=$time-$oldinfo["suspend_time"];
				//重新制定计划
				//predate0    predate1            predate2      predate3       predate4              predate5               predate6       predate6x     predate7 predate8      predate9  predate9x       //字符串
				//submit_time submit_approve_time research_time research_time1 research_approve_time research_approve_time1 cooperate_time planfile_time bid_time contract_time plan_time sendtask_time   //时间戳
				...
				//验收
				//predate100  predate101   predate102
				//finish_time finish_time1 activity_time
				
				//外线计划
				//outplantime1  outplantime2  outplantime3  outplantime4
				//outplantime1x outplantime2x outplantime3x outplantime4x
				$testtest["title"]=$timedistance;
				$testtest["content"]=$oldinfo["predate0"]."-".strtotime($oldinfo["predate0"]);
				M("Test")->add($testtest);
				
				if((empty($oldinfo["submit_time"]))&&(!empty($oldinfo["predate0"]))) $model->where($condition)->setField("predate0",date("Y-m-d",strtotime($oldinfo["predate0"])+$timedistance));
				if((empty($oldinfo["submit_approve_time"]))&&(!empty($oldinfo["predate1"]))) $model->where($condition)->setField("predate1",date("Y-m-d",strtotime($oldinfo["predate1"])+$timedistance));
				if((empty($oldinfo["research_time"]))&&(!empty($oldinfo["predate2"]))) $model->where($condition)->setField("predate2",date("Y-m-d",strtotime($oldinfo["predate2"])+$timedistance));
				if((empty($oldinfo["research_time1"]))&&(!empty($oldinfo["predate3"]))) $model->where($condition)->setField("predate3",date("Y-m-d",strtotime($oldinfo["predate3"])+$timedistance));
				if((empty($oldinfo["research_approve_time"]))&&(!empty($oldinfo["predate4"]))) $model->where($condition)->setField("predate4",date("Y-m-d",strtotime($oldinfo["predate4"])+$timedistance));
				if((empty($oldinfo["research_approve_time1"]))&&(!empty($oldinfo["predate5"]))) $model->where($condition)->setField("predate5",date("Y-m-d",strtotime($oldinfo["predate5"])+$timedistance));
				if((empty($oldinfo["cooperate_time"]))&&(!empty($oldinfo["predate6"]))) $model->where($condition)->setField("predate6",date("Y-m-d",strtotime($oldinfo["predate6"])+$timedistance));
				if((empty($oldinfo["planfile_time"]))&&(!empty($oldinfo["predate6x"]))) $model->where($condition)->setField("predate6x",date("Y-m-d",strtotime($oldinfo["predate6x"])+$timedistance));
				if((empty($oldinfo["bid_time"]))&&(!empty($oldinfo["predate7"]))) $model->where($condition)->setField("predate7",date("Y-m-d",strtotime($oldinfo["predate7"])+$timedistance));
				if((empty($oldinfo["contract_time"]))&&(!empty($oldinfo["predate8"]))) $model->where($condition)->setField("predate8",date("Y-m-d",strtotime($oldinfo["predate8"])+$timedistance));
				if((empty($oldinfo["plan_time"]))&&(!empty($oldinfo["predate9"]))) $model->where($condition)->setField("predate9",date("Y-m-d",strtotime($oldinfo["predate9"])+$timedistance));
				if((empty($oldinfo["sendtask_time"]))&&(!empty($oldinfo["predatex"]))) $model->where($condition)->setField("predatex",date("Y-m-d",strtotime($oldinfo["predatex"])+$timedistance));
				...
				{
					if((empty($val["realtimebegin"]))&&(!empty($oldinfo["plantimebegin"]))) M("Plmschedule")->where("id=".$val["id"])->setField("plantimebegin",date("Y-m-d",strtotime($oldinfo["plantimebegin"])+$timedistance));
					if((empty($val["realtimeend"]))&&(!empty($oldinfo["plantimeend"]))) M("Plmschedule")->where("id=".$val["id"])->setField("plantimeend",date("Y-m-d",strtotime($oldinfo["plantimeend"])+$timedistance));
				}
				
				if((empty($oldinfo["finish_time"]))&&(!empty($oldinfo["predate100"]))) $model->where($condition)->setField("predate100",date("Y-m-d",strtotime($oldinfo["predate100"])+$timedistance));
				if((empty($oldinfo["finish_time1"]))&&(!empty($oldinfo["predate101"]))) $model->where($condition)->setField("predate101",date("Y-m-d",strtotime($oldinfo["predate101"])+$timedistance));
				if((empty($oldinfo["activity_time"]))&&(!empty($oldinfo["predate102"]))) $model->where($condition)->setField("predate102",date("Y-m-d",strtotime($oldinfo["predate102"])+$timedistance));
				
				if((empty($oldinfo["outplantime1x"]))&&(!empty($oldinfo["outplantime1"]))) $model->where($condition)->setField("outplantime1",date("Y-m-d",strtotime($oldinfo["outplantime1"])+$timedistance));
				if((empty($oldinfo["outplantime2x"]))&&(!empty($oldinfo["outplantime2"]))) $model->where($condition)->setField("outplantime2",date("Y-m-d",strtotime($oldinfo["outplantime2"])+$timedistance));
				if((empty($oldinfo["outplantime3x"]))&&(!empty($oldinfo["outplantime3"]))) $model->where($condition)->setField("outplantime3",date("Y-m-d",strtotime($oldinfo["outplantime3"])+$timedistance));
				if((empty($oldinfo["outplantime4x"]))&&(!empty($oldinfo["outplantime4"]))) $model->where($condition)->setField("outplantime4",date("Y-m-d",strtotime($oldinfo["outplantime4"])+$timedistance));
