<?php
class XmtjAction extends CommonAction {		
	...
	*/
	...
	public function index() {
		...
		if($_REQUEST["moduletitle"]=="项目工作报告")
		{
			$this->indexproject();
			return;
		}
		if($_REQUEST["moduletitle"]=="部门工作报告")
		{
			$this->indexdept();
			return;
		}
		if($_REQUEST["moduletitle"]=="年度月度报告")
		{
			$this->indexyear();
			return;
		}
		...
		{
			if($_REQUEST["moduletitle"]=="运营工期分析")
			$_REQUEST['tab']=1;
			if($_REQUEST["moduletitle"]=="标准工期分析")
			$_REQUEST['tab']=16;
			if($_REQUEST["moduletitle"]=="延误原因分析")
			$_REQUEST['tab']=6;
			if($_REQUEST["moduletitle"]=="计划达成率分析")
			$_REQUEST['tab']=26;
			if($_REQUEST["moduletitle"]=="项目分类统计")
			$_REQUEST['tab']=41;
		}
		...
		if(($_REQUEST['tab']=="41")||($_REQUEST['tab']=="42")||($_REQUEST['tab']=="43")||($_REQUEST['tab']=="44"))
		{
			if($_REQUEST['tab']=="41")
			{
				//查找项目所有城市
				...
				foreach($cities as $key => $val)
				{
					$cities[$key]["para"] = $val[city];
					$mapforProject[city] = $val[city];
					$cities[$key][projects]=M("Project")->where($mapforProject)->field("city,design_status,activity")->select();
					
				}
				CommonmethodAction::getstatuscount($cities);
				$this->assign('cities',$cities);	
				$this->assign('para',"项目地域");
				$this->assign('searchpara',"city");
				if($_SESSION["app"])
				{
					$this->display("indexapp");
				}
			...
			if($_REQUEST['tab']=="42")
			{
				...
				$mapforRole["name"]=array("in","工程一部职员,工程二部职员");
				$roleid=M("Role")->where($mapforRole)->getField("id");
				$mapforUser["position"]=array("eq",$roleid);
				$mapforUser["status"]=array("eq","1");
				$cities=M("User")->where($mapforUser)->field("nickname,city,projecttype")->select();
				foreach($cities as $key => $val)
				{
					$cities[$key]["para"] = $val[nickname];
					$cities[$key]["gongchenguser"]=$val["nickname"];
					$mapforProject[gongchenguser] = array("in",$val["nickname"]);
					$cities[$key][projects]=M("Project")->where($mapforProject)->field("id,gongchenguser,design_status")->select();
				}
				CommonmethodAction::getstatuscount($cities);
				$this->assign('cities',$cities);
				$this->assign('para',"项目经理");
				$this->assign('searchpara',"gongchenguser");
				if($_SESSION["app"])
				{
					$this->display("indexapp");
				}
			...
			if($_REQUEST['tab']=="43")
			{
				$mapforProject["invester"]=array("neq","");
				$cities=M("Project")->where($mapforProject)->group("invester")->field("invester")->select();
				foreach($cities as $key => $val)
				{
					$cities[$key]["para"] = $val[invester];
					$mapforProject[invester] = $val[invester];
					$cities[$key][projects]=M("Project")->where($mapforProject)->field("invester,design_status,activity")->select();
				}
				CommonmethodAction::getstatuscount($cities);
				$this->assign('cities',$cities);	
				$this->assign('para',"项目地域");
				$this->assign('searchpara',"invester");
				if($_SESSION["app"])
				{
					$this->display("indexapp");
				}
	...
	{
		$map['name'] = array(array("like","%工程一部%"),array("like","%工程二部%"),array("like","%工程三部%"),array("like","%品质安全部%"),array("like","%开发%"),array("like","%供应链%"),array("like","%设计%"),"or");
		$map['status'] = 1;
		$name = "Dept";
		$model = D($name);
		$voList = $model->where($map)->select();
		...
		if(false!==strstr($deptinfo["name"],"品质安全")) $classify = "施工"; 
	...
	public function toexcel()
	{
		...
		for($i=0;$i<$number;$i++)
		{
			$data[$i]['classify']=str_replace("节点库","",$schedules[$i]['classify']);
			...
			$data[$i]['realtimebegin']=$schedules[$i]['realtimebegin'];
			$data[$i]['realtimeend']=$schedules[$i]['realtimeend'];
			
			
			$data[$i]['plantimelength']=$schedules[$i]['plantimelength'];
			$data[$i]['realtimelength']=$schedules[$i]['realtimelength'];
			$data[$i]['planpercent']=$schedules[$i]['planpercent'];
			$data[$i]['percent']=$schedules[$i]['percent'];
			
			if(($schedules[$i][planpercent]!="100%")&&($schedules[$i][percent]<$schedules[$i][planpercent]))
			{
				$data[$i][deviate]= "是";
			}
			else
			...
			if(($schedules[$i][planpercent]=="100%")&&($schedules[$i][percent]!="100%"))
			{
				$data[$i][delaydays]= $this->diffBetweenTwoDays($schedules[$i]["plantimeend"], $date);
			}
			else
			{
				$data[$i][delaydays]= "";
		...
		$file=$plminfo["title"]."-项目进度报表-".$date;
		$title='项目进度报表';
		$subtitle=$plminfo["title"]."-项目进度报表-".$date;
		
		$th_array=array('专项分类','节点分类','节点名称','计划开始时间','计划完成时间','实际开始时间','实际完成时间','计划天数','实际天数','计划百分比','实际百分比','偏离','超期');
