<?php
class PlmfileAction extends CommonAction {	
	...
	public function index() {
		...
		if($_REQUEST['tab']==100)
		{
			$folders=M("Classifyfolder")->where(array('status'=>0))->select();
		...
		if($_REQUEST['tab']==102)
		{
			...
			if(!empty($_REQUEST['plmid']))
			{
				...
				if(!empty($_REQUEST["classify"]))
				{
					$map["type"]=array("like","%".$_REQUEST["classify"]."%");
					$this->assign("classify",$_REQUEST["classify"]);
				}
				$this->assign("topclassify",$_REQUEST["topclassify"]);
				...
				if($_SESSION["app"])
				{
					$this->display("../Plmfile/indexapp");
				}
				else
				{
					$this->display("../Plmfile/index");
				}
				return;
			}
	...
	public function draft()
	{
		...
		$mapforClassifyfile["status"]=0;
		//$mapforClassifyfile["name"]=$_REQUEST["classify"];
		$classifies=M("Classifyfile")->where($mapforClassifyfile)->select();
		$this->assign('classifies',$classifies);
	...
	public function insert(){
		...
		$this->redirect('index','tab=102&plmid='.$_REQUEST[plmid].'/classify/'.$_REQUEST[classify]."/");
	}
...
//提交审核
    ...
    public function editfolder(){
    	$id=$_REQUEST['id'];
    	$name=M('classifyfolder')->where(array('id'=>$id))->getfield('name');
	...
	public function download_zip()
	{
		...
		*/
		...
		$classifies=M("Classifyfolder")->where(array('status'=>0))->select();
		foreach ($classifies as $key => $value)
		{
			$nofile="YES";
			$x=0;
