<?php
class MeetingscheduleAction extends CommonAction {
	//过滤查询字段
	...
	public function index() {
		...
		if($_REQUEST['calendar'] == 2){
			...
			$map['timebegin']=array(array('egt',$timebegin." 00:00"),array('elt',$timeend." 23:59"),'and');
		}
		$map['releaser'] = array('like','%'.$_SESSION['loginUserName'].'%');
		$worklist=M("Meetingassignment")->where($map)->order("timebegin asc")->select();
		...
		foreach ($data as $key => $val){
			foreach ($val as $key1 => $val1){
				...
				foreach ($worklist as $key2 => $val2){
					if((date("Y-m-d",strtotime($val2["timebegin"])) == $val1["date"])){
		...
		$type3 = M("meetingset")->where("type=3")->select();
		...
		/*
		$map = $this->_search('Meetingassignment');
		if (method_exists($this, '_filter')) {
			$this->_filter($map);
		}
		$map['user_id|current'] = array('like','%'.$_SESSION['loginUserName'].$_SESSION['number'].'%');
		//$map['status'] = 1;
		...
		if((!empty($_REQUEST['timebeginmywork']))&&(empty($_REQUEST['timeendmywork'])))
		{
			$map['create_time'] = array('gt',strtotime($_REQUEST['timebeginmywork']));
		}
		else if((empty($_REQUEST['timebeginmywork']))&&(!empty($_REQUEST['timeendmywork'])))
		{
			$map['create_time'] = array('lt',strtotime($_REQUEST['timeendmywork']));
		}
		else if((!empty($_REQUEST['timebeginmywork']))&&(!empty($_REQUEST['timeendmywork'])))
		{
			$map['create_time'] = array(array('gt',strtotime($_REQUEST['timebeginmywork'])),array('lt',strtotime($_REQUEST['timeendmywork'])),'and');
		}
	...
	public function foreverdelete() {
		$id=$_REQUEST['id'];
		$res=M('meetingassignment')->where('id='.$id)->delete();
		if($res){
			//删除所有会议决议
