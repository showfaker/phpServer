<?php
class PlmattendanceAction extends CommonAction {			
	public function index() {
        ...
        if($_REQUEST['tab']==2){
            ...
            if(!empty($_REQUEST['user'])){
				$map[worker]=array('like','%'.$_REQUEST[user].'%');
	...
	protected function _list($model, $map, $sortBy = '', $asc = false,$tab,$arr) {
        ...
        if ($count > 0) {
            ...
            if($tab==2){
				...
				$p->parameter .= "month=" . $_REQUEST["month"] . "&";
            }
            //分页显示
            $page = $p->show();
    ...
    public function insert(){
		...
		$workers=M('Plmworker')->where($mapPlmworker)->select();
		...
		*/
		...
		{
			$data[worker]=$val["worker"];
			$data[workerid]=$val["workerid"];
			$data[number]=$val["number"];
			$data[job]=$val["job"];
			$data[status]=$_REQUEST["status_".$val[workerid]];
			//$data[money]=$_REQUEST["money_".$val[workerid]];
			
			$mapforWorker["number"]=$val["number"];
			$data[money]=M("Worker")->where($mapforWorker)->getField("salary");
			
			$data[group]=$_REQUEST["group_".$val[workerid]];
			$data[groupid]=$_REQUEST["groupid_".$val[workerid]];
			$data['step']=$_REQUEST['step'];
			//$data[content]=$_REQUEST["content_".$val[workerid]];
			
			
			$mapforplmattendance["plmid"]=$_REQUEST['plmid'];
			$mapforplmattendance["groupid"]=$_REQUEST['groupid'];
			$mapforplmattendance["date"]=$_REQUEST['date'];
			$mapforplmattendance["workerid"]=$val["workerid"];
			$exist=M("plmattendance")->where($mapforplmattendance)->find();
			if($exist)
			{
				if($exist["step"]==$_REQUEST["step"])
				{
					M("plmattendance")->where("id=".$exist["id"])->setField("status",$_REQUEST["status_".$val[workerid]]);
				}
				else
				{
					if($exist["status"]=="在岗")
					...
					{
						M("plmattendance")->where("id=".$exist["id"])->delete();
						M('plmattendance')->add($data);
					}
				}
			}
			else
		...
		$mapforplmattendancemonth["month"]=$datearray[1];
		foreach($workers as $key => $val)
		{
			
			
			
			$mapforplmattendancemonth["workerid"]=$val["workerid"];
			$existid=M("plmattendancemonth")->where($mapforplmattendancemonth)->getField("id");
			if(!$existid)
			{
				$dataplmattendancemonth["month"]=$datearray[1];
				$dataplmattendancemonth["workerid"]=$val["workerid"];
				$dataplmattendancemonth[worker]=$val["worker"];
				$dataplmattendancemonth[number]=$val["number"];
				$dataplmattendancemonth[job]=$val["job"];
				$dataplmattendancemonth[attendance]=$daystr;
				
				$dataplmattendancemonth[groupid]=$val["groupid"];
				$dataplmattendancemonth[group]=$val["group"];
				
				$existid=M("plmattendancemonth")->add($dataplmattendancemonth);
			}
			$daystr=M("plmattendancemonth")->where("id=".$existid)->getField("attendance");
			$dayarray=explode(",",$daystr);
			
			$currentkey=$datearray[2]-1;
			
			$daystr="";
			//if($_REQUEST["status_".$val[workerid]]=="在岗")$dayarray[$currentkey]=1;
			//改成下面
			$mapforPlmattendance["date"]=$_REQUEST["date"];
			$mapforPlmattendance["workerid"]=$val["workerid"];
			$attendancedata=M('Plmattendance')->where($mapforPlmattendance)->find();
			if($attendancedata["status"]=="在岗")$dayarray[$currentkey]=1;else $dayarray[$currentkey]=0;
			
			for($i=0;$i<=31;$i++)
			{
				$daystr.=$dayarray[$i].",";
			}
			M("plmattendancemonth")->where("id=".$existid)->setField("attendance",$daystr);
	...
	public function toexcel()
	{
		...
		for($i=0;$i<$number;$i++)
		{
			...
			$data[$i]['worker']=$volist[$i]['worker'];
			$data[$i]['workerid']=$volist[$i]['workerid'];
			$data[$i]['job']=$volist[$i]['job'];
		...
		$th_array=array('出勤日期','项目名称','项目阶段','分组','人员姓名','人员编号','岗位','日薪','工作内容','出勤状态');
