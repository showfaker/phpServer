<?php
class MaterialsplmAction extends CommonAction {			
	public function index() {
		...
		if($_REQUEST['tab']==2){
			$model = M("Materialsplm");
			if(!empty($_REQUEST[plmid])){
				$map[plmid]=array('eq',$_REQUEST[plmid]);
				$this->assign("plmid",$_REQUEST[plmid]);
				$plminfo=M("Project")->where(array('id'=>$_REQUEST['plmid']))->find();
				if(!empty($_REQUEST[material]))
				{
					$map["name"]=array("like","%".$_REQUEST[material]."%");
					$this->assign("material",$_REQUEST[material]);
				}
				
				$materials=M("materialsplm")->where($map)->select();
	...
	protected function _list($model, $map, $sortBy = '', $asc = false,$tab,$arr) {
        ...
        if ($count > 0) {
            ...
            if($tab==2){
				...
				foreach ($voList as $key => $value) 
				{
					$map1[orderid]=$value[id];
					$materials=M("Plmmaterials")->where($map1)->order("supplier asc")->select();
					$price=0;
					foreach($materials as $key1=>$val1){
						$materials[$key1][total]=$val1[price]*$val1[count];
						$price+=$materials[$key1][total];
					}
					$voList[$key]["detail"]=$materials;
				}
	...
	public function insert(){
		...
		for($k=0;$k<count($_POST[para1]);$k++){
			$data1[name]=$_POST[para1][$k];
			$data1[brand]=$_POST[para2][$k];
			$data1[brandid]=M("brand")->where("name='".$_POST[para2][$k]."'")->getfield("id");
			$data1[standard]=$_POST[para3][$k];
			$data1[unit]=$_POST[para4][$k];
			$data1[price]=$_POST[para5][$k];
			$data1["density"]=$_POST[para7][$k];
			$data1[plmid]=$_POST[plmid];
			M("Materialsplm")->add($data1);
		}
	...
	*/
	...
	{	
		...
		for($k=0;$k<count($_POST[name]);$k++){
			...
			$data1[plmid]=$_POST[plmid];
			$data1[density]=$_POST[density][$k];
			$data1[ctime]=$time;
			$material_id=M("materialsplm")->add($data1);
		}
	...
	public function ajaxdelete(){
		M("materialsplm")->where("id='".$_POST[id]."'")->delete();
		echo 1;
	}
