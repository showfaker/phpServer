...
class CompanyAction extends CommonAction {
    ...
    public function add() {
    	$companyclassify=M("Companyclassify")->field('id,name')->select();
    	$this->assign('companyclassify', $companyclassify);
    	if($_SESSION[skin]!=3)
    	{
    		$this->display(addoa);
    	}
    ...
    function detail() {
    	...
    	$model = M($name);
    	$id = $_REQUEST ["id"];
		$mapforCompany["id|para1"]=array("eq",$id);
    	$vo = $model->where($mapforCompany)->find();
	...
	function ajax(){
		M("Company")->where("id='".htmlspecialchars($_REQUEST[id])."'")->delete();
