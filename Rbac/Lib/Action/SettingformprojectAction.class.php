<?php
class SettingformprojectAction extends CommonAction {
	//过滤查询字段
	...
	public function index() {
		...
		$volist=M("Settingformproject")->where($mapforSettingform)->order("sort asc")->select();
		foreach ($volist as $key => $val) {
			$mapforsettingform[id]=array("eq",$val[id]);
			$volist[$key][detail]=M("Settingformproject")->where($mapforsettingform)->order("create_time desc")->find();
			
			$mapforGroup[id]=array("eq",$val[pid]);
			$volist[$key][pname]=M("Group")->where($mapforGroup)->getField("title");
		}
	...
	function insert() {
		...
		$ifexist=M("Settingformproject")->where($mapforexist)->getField("id");
		if(!empty($ifexist))
		{
			$this->error('模块已经存在!');
		}
		
		$datanode["name"]="Aproject";
		$datanode["title"]=$_REQUEST["title"];
		$datanode["status"]="1";
		$datanode["remark"]="";
		$datanode["sort"]=$_REQUEST["sort"];
		$datanode["pid"]="1";
		$datanode["level"]="2";
		$datanode["type"]="2";
		$datanode["group_id"]=$_REQUEST["pid"];
		M("Node")->add($datanode);
	...
	function setsubmit() {
		...
		$oldinfo=M("Settingformproject")->where("id=".$_REQUEST["id"])->find();
		...
		if ($list !== false) { //保存成功
			...
			$mapforNode["name"]="Aproject";
			$mapforNode["title"]=$oldinfo["title"];
			$nodeid=M("Node")->where($mapforNode)->getField("id");
			
			
			$datanode["title"]=$_REQUEST["title"];
			$datanode["sort"]=$_REQUEST["sort"];
			$datanode["group_id"]=$_REQUEST["pid"];
			M("Node")->where("id=".$nodeid)->save($datanode);
	...
	function update() {
		...
		$repeatserial=M("Settingformproject")->where($mapforrepeatserial)->getField("id");
		if($repeatserial)
		{
			$this->error('流水号索引重复!');
		}
		...
		$oldinfo=M("Settingformproject")->where($mapforrepeat)->find();
		$repeat=$oldinfo["id"];
		...
		if($repeat)
		{
			...
			if(empty($model->help))
			{
				$model->help=M("Settingformproject")->where($mapforrepeat)->getField("help");
			}
			$model->sort=M("Settingformproject")->where($mapforrepeat)->getField("sort");
		...
		$settingforminfo=M("Settingformproject")->where("id=".$settingformid)->find();
		$Model = new Model();
		...
		{
			if($settingforminfo["tableannexname".$row])//($newinfo["tableparaname".$row])!=($oldinfo["tableparaname".$row])
			{
				$sql = "ALTER TABLE think_flow_".$settingforminfo[tablename]." add ".$settingforminfo["tableannexname".$row]." NVARCHAR(200) DEFAULT ''";// NOT NULL 
				$Model->query($sql);
				$sql = "ALTER TABLE think_flow_".$settingforminfo[tablename]." add ".$settingforminfo["tableannexname".$row]."realname NVARCHAR(200) DEFAULT ''";// NOT NULL 
				$Model->query($sql);
			}
		}
	...
	function edit() 
	{
		...
		$vo=M("Settingformproject")->where($mapforsetting)->find();
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
                ...
                if (false !== $model->where($condition)->delete())
				{
                    $mapforNode["name"]="Aproject";
					$mapforNode["title"]=$oldinfo["title"];
					$nodeinfo=M("Node")->where($mapforNode)->find();
					M("Node")->where("id=".$nodeinfo["id"])->delete();
                    $this->success('删除成功！');
                } else {
                    $this->error('删除失败！');
                }
