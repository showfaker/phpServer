<?php
class SecondcancelAction extends CommonAction {
	//过滤查询字段
	...
	public function index() {
		...
		*/
		...
		$map[design_status]=array("in","取消");//完成验收,暂停中
	...
	public function resume() {
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
                $condition = array($pk => array('in', explode(',', $id)));
                if (false !== $model->where($condition)->setField("design_status","立项中"))
				{
                    //echo $model->getlastsql();
					$oldinfo=M("Project")->where($condition)->find();
					$date=date('Y-m-d H:i');
					$handlehistory=$oldinfo['handlehistory'].$_SESSION['loginUserName']."恢复取消项目"."</br>时间：".$date."</br>------------------</br>";
