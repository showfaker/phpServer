<?php
class QdxycheckAction extends CommonAction {			
	//过滤查询字段
	function _filter(&$map){
		$map['projecttype'] = array("neq","承揽项目");
		$map['step3'] = array("egt","0.5");
		...
		{
			$plmbidarray=M("Plmcooperate")->where($map1)->select();
	...
	public function index() {
		...
		if($_REQUEST['client'])
		{
			$map['client'] = array('like',"%".$_POST['client']."%");
			$this->assign("client",$_POST['client']);
		}
		...
		if($_SESSION[app]=="1")
		{
			$this->display("../Qdxycheck/indexapp");
			return;
		}
		$this->display("../Qdxycheck/index");
		return;
	}
	...
	protected function _list($model, $map, $sortBy = '', $asc = false) {
        ...
        if ($count > 0) {
			...
			foreach($voList as $key => $val)
			{
				...
				$mapforPlmcooperate[plmNumber]=$val["id"];
				$voList[$key][plminfo]=M("Plmcooperate")->where($mapforPlmcooperate)->find();
				...
				$voList[$key][plminfo]['contractrecord']=explode(',',$voList[$key][plminfo]['contractrecord']);
				$voList[$key][plminfo]['contractrecordfilename']=explode(',',$voList[$key][plminfo]['contractrecordfilename']);
	...
	function draftfirst() {
		...
		$mapforPlmcooperate[plmNumber]=$vo["id"];
		$plminfo=M("Plmcooperate")->where($mapforPlmcooperate)->find();
		$plminfo['contract']=explode(',',$plminfo['contract']);
		$plminfo['contractfilename']=explode(',',$plminfo['contractfilename']);
		
		$plminfo['contractrecord']=explode(',',$plminfo['contractrecord']);
		$plminfo['contractrecordfilename']=explode(',',$plminfo['contractrecordfilename']);
	...
	function approvedetail() {
		...
		$this->display("../Qdxy/approvedetail");
	}
	function draft() {
		$name = "Project";
		$model = M($name);
	...
	function insert() {
		...
		M("Project")->where("id='" . $plminfo[id] . "'")->setField("step3","1");
		M("Project")->where("id='" . $plminfo[id] . "'")->setField("keeponrecordmoney",$_REQUEST["keeponrecordmoney"]);
		...
		if(empty($_REQUEST[approve]))
		{
			...
			$model->cooperate_time=time();
			$model->client=$_REQUEST["para7"];
			$model->clienttel=$_REQUEST["para8"];
			$model->handlehistory=$info['handlehistory'].$_SESSION['loginUserName']."于".$date."上传了合作协议</br>------------------</br>"; 
		}
		else
		{
	...
	function approvesubmit() {
		...
		if(($_REQUEST[result]=="同意"))/*同意*/
		{
			...
			if(($info[approvestatus]=="1")||($info[approvestatus]=="2"))
			{
				...
				$data['href'] ="index.php?s=Qdxy/index";
				$data['taskid'] =$taskid;
				$data['type'] ="Qdxy";
