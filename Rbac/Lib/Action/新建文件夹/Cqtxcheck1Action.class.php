<?php
class Cqtxcheck1Action extends CommonAction {			
	//过滤查询字段
	...
	function insert() {
		...
		if(!empty($_FILES['file']['name'][0]))
		{
			...
			M("Project")->where($mapforProject)->setField("intention",$newnameall);
			M("Project")->where($mapforProject)->setField("intentionfilename",$filenameall);
		}
		
		$model->id=$_REQUEST["id"];;
		$model->save();
		if(empty($plminfo["step4"]))
		{
