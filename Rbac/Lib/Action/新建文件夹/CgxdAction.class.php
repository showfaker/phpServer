<?php
class CgxdAction extends CommonAction {			
	public function index() {
		...
		if($_REQUEST['tab']==2){
			...
			if($_SESSION["role"]=="采购供应商")
			{
				$mapPlmmaterials[supplier]=array("eq",$_SESSION[nickname]);
				$this->assign("supplier",$_SESSION[nickname]);
				$orderarray=M("Plmmaterials")->where($mapPlmmaterials)->group("orderid")->select();
				foreach($orderarray as $key => $val)
				{
					$orders.=$val[orderid].",";
	...
	{	
		for($k=0;$k<count($_POST[number]);$k++){
			...
			if(($k!=0)&&($supplier!=$_POST[supplier][$k-1]))
			{
				echo 2;die;
			}
		}
	...
	public function subinsert(){
		...
		$data[supplier]=$_POST[supplier][0];
		...
		$date=date('Y-m-d H:i');
		$scheduledata['content']=$_SESSION['loginUserName']."于".$date."提交采购下单，请您审批。";
	...
	public function ajaxsubmit1(){
		$info=M('plmmaterialorder')->find($_POST['id']);
		
		if(empty($info["filename"])||empty($info["filename1"]))
		{
			echo 0;
			return;
		}
		...
		$date=date("Y-m-d");
		$data['sendstatus']=0.3;
		$data['handle']=$info['handle'].$date.'--'.$_SESSION['loginUserName'].'提交了验收申请;';
		M("plmmaterialorder")->where("id='".$_POST[id]."'")->save($data);
		...
		$date=date('Y-m-d H:i');
		$scheduledata['content']=$_SESSION['loginUserName']."于".$date."提交验收申请，请您审批。";
		$scheduledata['href'] ="index.php?s=Cgxdapprove/index";
		$scheduledata['taskid'] =$info[id];
		$scheduledata['type'] ="Cgxd";
		$userschedule=$this->findleader();
	...
	public function ajaxsubmit2(){
		$info=M('plmmaterialorder')->find($_POST['id']);
		$date=date("Y-m-d");
		$data['sendstatus']=0.1;
		$data['handle']=$info['handle'].$date.'--'.$_SESSION['loginUserName'].'确认接受订单;';
		M("plmmaterialorder")->where("id='".$_POST[id]."'")->save($data);
	...
	public function ajaxsubmit3(){
		$info=M('plmmaterialorder')->find($_POST['id']);
		$date=date("Y-m-d");
		$data['sendstatus']=0.2;
		$data['handle']=$info['handle'].$date.'--'.$_SESSION['loginUserName'].'确认发货;';
		M("plmmaterialorder")->where("id='".$_POST[id]."'")->save($data);
		...
		$date=date('Y-m-d H:i');
		$scheduledata['content']=$_SESSION['loginUserName']."于".$date."确认发货，请您提交采购验收相关材料";
		$scheduledata['href'] ="index.php?s=Cgxd/index";
		$scheduledata['taskid'] =$info[id];
		$scheduledata['type'] ="Cgxd";
		$userschedule=$this->findUserByName($info["user"]);
		$scheduledata['user']=$userschedule['nickname'].$userschedule['number'];
		$this->Addschedule($scheduledata);
	...
	public function uploadupdate(){
		...
		foreach ($_FILES['file1'][name] as $key => $value) {
			if(!empty($_FILES['file1']['name'][$key]))
			{
				$map[id]=array("in",$_POST[id]);			
				$orderinfo=M("plmmaterialorder")->where($map)->field("id,newname1,filename1")->find();
				$savePath = '../Public/Uploads/';     //设置附件上传目录
				$ext = strtolower(end(explode(".",basename($_FILES['file1']['name'][$key])))); 
				$uuid=uniqid(rand(), false).$key;//防止生成相同文件名id
				$newname = $uuid.'.'.$ext;
				$upload_file = $savePath.$newname;
				...
				$res1=move_uploaded_file($_FILES['file1']['tmp_name'][$key],$upload_file);
				if($res1){
					if(!empty($orderinfo[newname1])){
						$data3[newname1] = $orderinfo[newname1].','.$newname;
						$data3[filename1] = $orderinfo[filename1].','.$_FILES['file1']['name'][$key];
					}else{
						$data3[newname1] = $newname;
						$data3[filename1] = $_FILES['file1']['name'][$key];
					}
				}
				

				M("plmmaterialorder")->where("id='".$_POST[id]."'")->save($data3);
				$tj=M('plmmaterialtj')->where(array('orderid'=>$_POST[id]))->select();
				foreach ($tj as $key => $value) {
					if(!empty($value[newname])){
						$data4[newname1] = $value[newname1].','.$newname;
						$data4[filename1] = $value[filename1].','.$_FILES['file1']['name'][$key];
					}else{
						$data4[newname1] = $newname;
						$data4[filename1] = $_FILES['file1']['name'][$key];
					}
					M("plmmaterialtj")->where("id='".$value[id]."'")->save($data4);
				}
