...
class ReportAction extends CommonAction {			
	...
	protected function _list($model, $map, $sortBy = '', $asc = false) {
        ...
        if ($count > 0) {
			...
			$worktypes=M("Plmworktype")->group("pworktype")->field("pworktype")->select();
			foreach($voList as $key => $val)
			{
				$mapforPlmcontract["plmNumber"] = $val[id];
				$contractinfo=M("Plmcontract")->where($mapforPlmcontract)->find();
				$voList[$key]["contractinfo"]=$contractinfo;
				$voList[$key]["quantities"]=$contractinfo[quantities1]+$contractinfo[quantities2];
				
				
				foreach($worktypes as $key1 => $val1)
				{
					$mapforplmoutputdaily["plmid"]=$val[id];
					$mapforplmoutputdaily["pworktype"]=$val1[pworktype];
					$voList[$key]["worktype"][$key1][output]=M("plmoutputdaily")->where($mapforplmoutputdaily)->sum("value");
				}
			}
