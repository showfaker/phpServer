<?php
class Ysgl1Action extends CommonAction {
	...
	public function index() {
		...
		//$map[design_status]=array("in","可研评审报告审批通过,合同审核完成,设计待审核,设计审核退回,设计审核通过,施工计划待审核,施工计划审核退回,施工计划审核通过,待施工,施工中,完成施工,暂停中");//新加的  可研评审报告审批通过
	...
	protected function _list($model, $map, $sortBy = '', $asc = false) {
        ...
        if ($count > 0) {
			...
			foreach($voList as $key => $val)
			{
				...
				$voList[$key]['ifsetworktype']=strpos($val["handlehistory"],"设置专项节点");
			}
	...
	function insert2() {
		...
		{
			if(($key!="id")&&($key!="gz")&&($key!="moduletitle"))
			{
	...
	function insert3() {
		...
		$handlehistory=$info['handlehistory'];
		$date=date('Y-m-d H:i:s');
		$handlehistory.=$_SESSION['loginUserName']."于".$date."变更专项节点</br>------------------</br>"; 
		$model->handlehistory=$handlehistory;
		$model->ysuser=$_SESSION['loginUserName'];
		$worktype=M("Plmworktype")->where("plmid=".$info[id])->order("id asc")->select();
		if(1)//!empty($info[illustration])&&!empty($info[drawing])
		{
			...
			if(empty($scheduleexist))
			{	
				$data['content']=$_SESSION['loginUserName']."于".$date."创建了《".$address."》项目专项节点变更，请您审核。";
				$data['href'] ="index.php?s=Ysgl/index";
				$data['taskid'] =$info[id];
				$data['type'] ="Ysgl";
				$userschedule=$this->findleader($info['projecttype'],$info['city']);
		...
		M("Plmworktypetemp")->where("plmid=".$_REQUEST["id"])->delete();
		$dataplmworktype["plmid"]=$_REQUEST["id"];
		$postdata=$_POST;
		foreach($postdata as $key => $val)
		{
	...
	function approvesubmit4() {
		...
		if($_REQUEST[approvestatus]=="1")
		{
			...
			$handlehistory.=$_SESSION['loginUserName']."于".$date."审核节点变更，审核结果：通过"."</br>意见：".$_REQUEST['suggestion']."</br>时间：".$date."</br>------------------</br>";
			
			
			$data['content']=$_SESSION['loginUserName']."于".$date."对《".$info['title']."》进行节点变更审核，结果：同意。";
			$data['receiver']=$info['ysuser'].$this->findNumberByNameAndRole($info['ysuser'],"设计师").",";
			$data['sender']="系统通知";
			$data['title'] =$_SESSION['loginUserName']."于".$date."对《".$info['title']."》进行节点变更审核，结果：同意。";
			$this->Sendmail($data);
			
			//审核变更
			$worktypetemps=M("Plmworktypetemp")->where("plmid=".$info[id])->order("id asc")->select();
			M("Plmworktype")->where("plmid=".$info[id])->delete();
			...
			$worktypetemps=M("Plmworktypetemp")->where("plmid=".$info[id])->delete();
		...
		else
		{
			...
			$handlehistory.=$_SESSION['loginUserName']."于".$date."审核节点变更，审核结果：退回"."</br>意见：".$_REQUEST['suggestion']."</br>时间：".$date."</br>------------------</br>";
			
			$data['content']=$_SESSION['loginUserName']."于".$date."对《".$info['title']."》进行节点变更审核，结果：退回。";
			$data['receiver']=$info['ysuser'].$this->findNumberByNameAndRole($info['ysuser'],"设计师").",";
			$data['sender']="系统通知";
			$data['title'] =$_SESSION['loginUserName']."于".$date."对《".$info['title']."》进行节点变更审核，结果：退回。";
			$this->Sendmail($data);
		...
		if ($list !== false) { //保存成功
			...
			{
				$this->redirect('index',array('tab'=>"6"));
			}
		} else {
			//失败提示
			$this->error('新增失败!');
