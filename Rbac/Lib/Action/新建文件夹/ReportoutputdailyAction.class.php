<?php
class ReportoutputdailyAction extends CommonAction {			
	public function index() {
		...
		$mapforProject[step6]=array('eq','1');
		$projects=M("Project")->where($mapforProject)->select();
		foreach($projects as $key=>$value ){
			
			$projects[$key]["groups"]=M("Plmgroup")->where("plmid=".$value[id])->select();
		...
		$plmoutputdaily=M("plmoutputdaily")->where($mapforplmoutputdaily)->order("id asc")->select();
		
		
		foreach($plmoutputdaily as $key=>$value ){
			$plmoutputdailydata[$value["date"]][$value["pworktype"]][$value["worktype"]]=$value["value"];
		}
		$this->assign("plmoutputdailydata",$plmoutputdailydata);
		foreach($worktypes as $key=>$value ){
			foreach($worktypes[$key]["subworktypes"] as $key1=>$value1)
			{
				foreach($plmoutputdaily as $key2=>$value2 ){
					...
					{
						$worktypes[$key]["subworktypes"][$key1]["total"]+=$plmoutputdailydata[$value2["date"]][$value2["pworktype"]][$value2["worktype"]];
					}
				}
				$worktypes[$key]["subworktypes"][$key1]["total1"]=$worktypes[$key]["subworktypes"][$key1]["total"]*$worktypes[$key]["subworktypes"][$key1]["price"];
			}
		}
		...
		$mapforPlmgroup[plmid]=$_REQUEST['plmid'];
		$groups=M("Plmgroup")->where($mapforPlmgroup)->select();
		...
		$jobs=M("Worker")->group("job")->field("job")->select();
	...
	public function insert(){
		...
		$mapforPlmworktype[plmid]=$_REQUEST['plmid'];
		$worktypes=M("Plmworktype")->where($mapforPlmworktype)->order("id asc")->select();
		foreach($worktypes as $key=>$value ){
			
			
			$value[remark]=$_REQUEST['remark_'.$value["id"]];
			$value[unit]=$_REQUEST['unit_'.$value["id"]];
			$value["count"]=$_REQUEST['count_'.$value["id"]];
			$value[price]=$_REQUEST['price_'.$value["id"]];
			
			M('Plmworktype')->save($value);
		}
		...
		/*
		$mapforPlmoutputdaily["plmid"]=$plminfo[id];
		$mapforPlmoutputdaily["groupid"]=$_REQUEST[groupid];
		M('Plmoutputdaily')->where($mapforPlmoutputdaily)->delete();
		
		$datearray=$this->printDates($plminfo["construction_start"],$plminfo["finishtime"]);
		foreach($datearray as $key=>$value ){
			foreach($worktypes as $key1=>$value1 ){
				$data["plmid"]=$_REQUEST['plmid'];
				$data["groupid"]=$_REQUEST['groupid'];
				$data["date"]=$value;
				$data[value]=$_REQUEST['value_'.$value."_".$value1["id"]];
				$data[worktype]=$value1['title'];
				$data[pworktype]=$value1['pworktype'];
				M('Plmoutputdaily')->add($data);
		...
		*/
		...
		foreach($worktypes as $key1=>$value1 ){
			...
			$data[value]=$_REQUEST['count_'.$value1["id"]];
			$data[price]=$_REQUEST['price_'.$value1["id"]];
			$data[money]=round($data[value]*$data[price],2);
			$data[worktype]=$value1['title'];
			$data[pworktype]=$value1['pworktype'];
			$data["duty"]=$_REQUEST["plan7"];
			M('Plmoutputdaily')->add($data);
		}
		
		
		
		$mapforPlmplan["plmid"]=$_REQUEST["plmid"];
		$mapforPlmplan["date"]=$_REQUEST["date"];
		$oldinfo=M("Plmplan")->where($mapforPlmplan)->find();
		
		$plminfo=M("Project")->where("id=".$_REQUEST["plmid"])->find();
	
		if(empty($oldinfo))
		{
			$dataplmplan["plmid"]=$_REQUEST["plmid"];
			$dataplmplan["plm"]=$plminfo["title"];
			$dataplmplan["date"]=$_REQUEST["date"];
			$dataplmplan["plan7"]=$_REQUEST["plan7"];
			$dataplmplan["user"]=$_SESSION["name"];
			$dataplmplan["create_time"]=time();
			M("Plmplan")->add($dataplmplan);
		}
		else
		{
			$dataplmplan["id"]=$oldinfo["id"];
			$dataplmplan["plmid"]=$_REQUEST["plmid"];
			$dataplmplan["plm"]=$plminfo["title"];
			$dataplmplan["date"]=$_REQUEST["date"];
			$dataplmplan["plan7"]=$_REQUEST["plan7"];
			$dataplmplan["user"]=$_SESSION["name"];
			$dataplmplan["create_time"]=time();
			M("Plmplan")->save($dataplmplan);
		}
		
		
		//设置形象进度
		$mapforPlmplan1["plmid"]=$_REQUEST["plmid"];;
		$plan1all=M("Plmplan")->where($mapforPlmplan1)->sum("plan1");
		
		$mapforPlmoutputdaily1["plmid"]=$_REQUEST['plmid'];
		$mapforPlmoutputdaily1["pworktype"]=array("like","%热再生%");
		$realall=M('Plmoutputdaily')->where($mapforPlmoutputdaily1)->sum("value");
		
		$percent=100*$realall/$plan1all;
		if($percent>100)$percent=100;

		M("Project")->where("id=".$_REQUEST['plmid'])->setField("image_progress",round($percent,2));
		
		$this->redirect('index','plmid='.$plminfo[id].'&groupid='.$_REQUEST[groupid].'/&tab=5/');
	}
