<?php
class GysglAction extends CommonAction {
	
	public function index() { 
		if(empty($_REQUEST['tab']))$_REQUEST['tab']=3;
		...
		{			
            ...
            if($_REQUEST['tab']==3){
				...
				if($_SESSION[role]=='采购供应商'){
					$map["supplier"]=$_SESSION["nickname"];
				}
				if(!empty($_REQUEST['brands']))
				{
	...
	public function update(){		
		$data[name]=$_POST[name];
		M("brand")->where("id='".$_POST[id]."'")->save($data);
		$this->redirect('index','tab=5');
	}
	...
	public function draft2(){
		...
		$brands=M("brand")->field("id,name")->select();
	...
	public function insert2(){
		...
		$data[brandid]=M('brand')->where(array('name'=>$_POST['brand']))->getfield('id');
		$data[ctime]=time();
		$data[status]=0;
		...
		if(!empty($_FILES['file1']['name'][0]))
		{
			...
			$data[file1]=$newnameall;
			$data[file1name]=$filenameall;
		}
		...
		M("materials")->where("id=".$res)->save($data);
	...
	public function ajax1(){
		$name=M("brand")->where("name='".$_POST[name]."'")->find();
		if(empty($name)){
			echo 1;
		}else{ 
			echo "品牌已存在";
	...
	public function foreverdelete1(){
		M("brand")->where("id='".$_POST[id]."'")->delete();
		echo 1;
	}
