<?php
class ComprehensiveAction extends CommonAction {			
	public function index() {
		...
		{			
			$mapforProject["title"]=array("eq",$_REQUEST['address']);
			$_REQUEST[plmid]=M("Project")->where($mapforProject)->getField("id");			
		...
		{
			$timebegin=$plminfo["construction_start"];
		}
		...
		{
			$timeend=$plminfo["finishtime"];
		}
		$this->assign("timebegin",$timebegin);
		$this->assign("timeend",$timeend);
		$datearray=$this->printDates($timebegin,$timeend);
		$this->assign("datearray",$datearray);
		$this->assign("datearraycount",2*count($datearray));
		
		$mapforPlmworktype["plmid"]=$_REQUEST["plmid"];
		$steps=M("Plmworktype")->where($mapforPlmworktype)->group("pworktype")->field("pworktype")->select();
		
		
		$mapforPlmworker["plmid"]=$_REQUEST["plmid"];
		$jobs=M("Plmworker")->where($mapforPlmworker)->group("job")->field("job")->select();
		foreach($jobs as $key=>$val)
		{
			foreach($steps as $key1=>$val1)
			{	
				$mapforPlmattendance[plmid]=$_REQUEST["plmid"];
				$mapforPlmattendance[job]=$val[job];
				$mapforPlmattendance[step]=$val1[pworktype];
				$mapforPlmattendance[status]="在岗";
				foreach($datearray as $key2=>$val2)
				{
					$mapforPlmattendance["date"]=$val2;
					$jobs[$key][$val1[pworktype]]["count"][$val2]=M("Plmattendance")->where($mapforPlmattendance)->count();
					$jobs[$key]["count"]+=$jobs[$key][$val1[pworktype]]["count"][$val2];
				}
			...
			$mapforPlmattendance[job]=$val[job];
			$mapforPlmattendance[step]=$val1[pworktype];
			$mapforPlmattendance[status]="离岗";
			$jobs[$key]["count1"]=M("Plmattendance")->where($mapforPlmattendance)->count();
		}
		$this->assign("jobs",$jobs);
		$this->assign("jobcount",count($jobs));
		
		
		
		$mapforPlmdevice["plmid"]=$_REQUEST["plmid"];
		$devices=M("Plmattendancedevice")->where($mapforPlmdevice)->group("deviceid")->select();
		foreach($devices as $key=>$val)
		{
			foreach($steps as $key1=>$val1)
			{	
				$mapforPlmattendancedevice[plmid]=$_REQUEST["plmid"];
				$mapforPlmattendancedevice[deviceid]=$val[deviceid];
				$mapforPlmattendancedevice[step]=$val1[pworktype];
				$mapforPlmattendancedevice[status]="在岗";
				foreach($datearray as $key2=>$val2)
				{
					$mapforPlmattendancedevice["date"]=$val2;
					$deviceattendances=M("Plmattendancedevice")->where($mapforPlmattendancedevice)->select();
					$devices[$key][$val1[pworktype]]["count"][$val2]=0;
					$devices[$key][$val1[pworktype]]["money"][$val2]=0;
					foreach($deviceattendances as $key3=>$val3)
					{
						$devices[$key][$val1[pworktype]]["count"][$val2]++;
						$devices[$key][$val1[pworktype]]["money"][$val2]+=$val3["money"];
					}
				
					$devices[$key]["count"]+=$devices[$key][$val1[pworktype]]["count"][$val2];
					$devices[$key]["money"]+=$devices[$key][$val1[pworktype]]["money"][$val2];
				
					$devices[$key]["price"]=$devices[$key]["money"]/$devices[$key]["count"];
				}
			}
			$mapforPlmattendancedevice[plmid]=$_REQUEST["plmid"];
			$mapforPlmattendancedevice[deviceid]=$val[deviceid];
			$mapforPlmattendancedevice[step]=$val1[pworktype];
			$mapforPlmattendancedevice[status]="离岗";
			$devices[$key]["count1"]=M("Plmattendancedevice")->where($mapforPlmattendancedevice)->count();
		}
		$this->assign("devices",$devices);
		$this->assign("devicecount",count($devices));
		
		
		$mapforPlmmaterials[plmid]=$_REQUEST["plmid"];
		$materials=M("Plmmaterials")->where($mapforPlmmaterials)->group("name")->select();
		foreach($materials as $key=>$val)
		{
			$materials[$key]["count"]=0;
			$materials[$key]["money"]=0;
			foreach($steps as $key1=>$val1)
			{	
				$mapforPlmmaterials[plmid]=$_REQUEST["plmid"];
				$mapforPlmmaterials[name]=$val[name];
				$mapforPlmmaterials[step]=$val1[pworktype];
				foreach($datearray as $key2=>$val2)
				{
					$mapforPlmmaterials["date"]=$val2;
					$materiallist=M("Plmmaterials")->where($mapforPlmmaterials)->select();
					$materials[$key][$val1[pworktype]]["count"][$val2]=0;
					$materials[$key][$val1[pworktype]]["money"][$val2]=0;
					foreach($materiallist as $key3=>$val3)
					{
						$materials[$key][$val1[pworktype]]["count"][$val2]+=$val3["count"];
						$materials[$key][$val1[pworktype]]["money"][$val2]+=$val3["count"]*$val3["price"];
					}
					
					$materials[$key][$val1[pworktype]]["count"][$val2]=round($materials[$key][$val1[pworktype]]["count"][$val2],2);
					$materials[$key][$val1[pworktype]]["money"][$val2]=round($materials[$key][$val1[pworktype]]["money"][$val2],2);
					
					$materials[$key]["count"]+=$materials[$key][$val1[pworktype]]["count"][$val2];
					$materials[$key]["money"]+=$materials[$key][$val1[pworktype]]["money"][$val2];
				}
			}
			
			$materials[$key]["count"]=round($materials[$key]["count"],2);
			$materials[$key]["money"]=round($materials[$key]["money"],2);
			$materials[$key]["price"]=round($materials[$key]["money"]/$materials[$key]["count"],2);
			
		}
		$this->assign("materials",$materials);
		$this->assign("materialcount",count($materials));
		...
		$classifiestemp=M("Classify")->where($mapforClassify)->select();
		$nametemp="";
		foreach($classifiestemp as $key=>$val)
		{
			$nametemp.=$val["name"].",";
		}
		
		$mapforPlmorder2paytime1["classify"]=array("in",$nametemp);
		$mapforPlmorder2paytime1[plmid]=$_REQUEST["plmid"];
		$classifies1=M("Plmorder2paytime")->where($mapforPlmorder2paytime1)->group("classify")->select();
		//总产值
		$mapforPlmoutputdaily["plmid"]=$_REQUEST["plmid"];
		$totalmoney=M("Plmoutputdaily")->where($mapforPlmoutputdaily)->sum("money");
		
		foreach($classifies1 as $key=>$val)
		{
			
			$mapforPlmorder2paytime_1[classify]=$val[classify];
			$classifies1[$key]["money"]=M("Plmorder2paytime")->where($mapforPlmorder2paytime_1)->sum("money");
			//$classifies2[$key]["money"]=0;
			foreach($steps as $key1=>$val1)
			{	
				/*
				$mapforPlmorder2paytime[classify]=$val[classify];
				$mapforPlmorder2paytime[step]=$val1[pworktype];
				$classifies1[$key][$val1[pworktype]]["money"]=M("Plmorder2paytime")->where($mapforPlmorder2paytime)->sum("money");
				$classifies1[$key]["money"]+=$classifies1[$key][$val1[pworktype]]["money"];
				*/
				$mapforPlmoutputdailyforstep["plmid"]=$_REQUEST["plmid"];
				$mapforPlmoutputdailyforstep["pworktype"]=$val1[pworktype];
				
				foreach($datearray as $key2=>$val2)
				{
					$mapforPlmoutputdailyforstep["date"]=$val2;
					$pworktypemoney=M("Plmoutputdaily")->where($mapforPlmoutputdailyforstep)->sum("money");
					$steps[$key1]["money"][$val2]=$pworktypemoney;
					$classifies1[$key][$val1[pworktype]]["money"]=round($classifies1[$key]["money"]*$pworktypemoney/$totalmoney,2);
				}
			}
			
		}
		$this->assign("classifies1",$classifies1);
		$this->assign("classify1count",count($classifies1));
		
		
		
		
		
		$mapforClassify["topname"]="其他费用";
		$classifiestemp=M("Classify")->where($mapforClassify)->select();
		$nametemp="";
		foreach($classifiestemp as $key=>$val)
		{
		...
		$mapforPlmorder2paytime1["classify"]=array("in",$nametemp);
		$mapforPlmorder2paytime1[plmid]=$_REQUEST["plmid"];
		$classifies2=M("Plmorder2paytime")->where($mapforPlmorder2paytime1)->group("classify")->select();
		foreach($classifies2 as $key=>$val)
		{
			$mapforPlmorder2paytime[classify]=$val[classify];
			$classifies2[$key]["money"]=M("Plmorder2paytime")->where($mapforPlmorder2paytime)->sum("money");
			//$classifies2[$key]["money"]=0;
			foreach($steps as $key1=>$val1)
			{	
				/*
				$mapforPlmorder2paytime[classify]=$val[classify];
				$mapforPlmorder2paytime[step]=$val1[pworktype];
				$classifies2[$key][$val1[pworktype]]["money"]=M("Plmorder2paytime")->where($mapforPlmorder2paytime)->sum("money");
				$classifies2[$key]["money"]+=$classifies2[$key][$val1[pworktype]]["money"];
				*/
				$mapforPlmoutputdailyforstep["plmid"]=$_REQUEST["plmid"];
				$mapforPlmoutputdailyforstep["pworktype"]=$val1[pworktype];
				$pworktypemoney=M("Plmoutputdaily")->where($mapforPlmoutputdailyforstep)->sum("money");
				$classifies2[$key][$val1[pworktype]]["money"]=round($classifies2[$key]["money"]*$pworktypemoney/$totalmoney,2);
			}
		...
		$this->assign("classifies2",$classifies2);
		$this->assign("classify2count",count($classifies2));
		
		
		$this->assign("steps",$steps);
		$this->assign("stepcount",count($steps));
		$this->assign("totalmoney",$totalmoney);//总产值
