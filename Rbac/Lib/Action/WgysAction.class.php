<?php
class WgysAction extends CommonAction {			
	//过滤查询字段
	...
	public function index() {
		...
		$map[design_status]=array("in","立项中,待施工,施工中,完成施工,施工完成,联合验收中,联合验收通过,竣工待验收,项目待验收,验收审核退回,完成验收");
	...
	function insert4() {
		...
		if(!empty($_FILES['file1']['name'][0]))
		{
			...
			$handlehistory.=$_SESSION['loginUserName']."于".$date."修改项目完成时间</br>------------------</br>"; 
		}
		$model->handlehistory=$handlehistory;
	...
	public function setback() {
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
                $condition = array($pk => array('in', explode(',', $id)));
				$info=$model->where($condition)->find();
                if (false !== $model->where($condition)->setField("design_status","施工中"))
				{
                    $this->success('退回成功！');
                } else {
                    $this->error('退回失败！');
	...
	public function advancecomplete() {
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
				...
				{
					/*
					if(empty($val["realtimebegin"])||empty($val["realtimeend"])||($val["percent"]!=="100%"))
					{
						$this->error('还有节点未完成100%，无法操作施工完成！');
					}*/
				}
				...
				*/
				...
				if(($info["design_status"]=="待施工")||($info["design_status"]=="施工中"))
				{
					$model->where($condition)->setField("design_status","施工完成");
					$model->where($condition)->setField("finish_time",date("Y-m-d",time()));
				}
