<?php
class BackmoneyAction extends CommonAction {
	//过滤查询字段
	...
	function insert() {
		...
		if($_REQUEST["backmoney"])
		{
			$dataplmdiscuss["plmid"]=$_REQUEST[id];
			$dataplmdiscuss["create_time"]=time();
			$dataplmdiscuss["backmoney"]=$_REQUEST[backmoney];
			$dataplmdiscuss["backtime"]=$_REQUEST["backtime"];
			$dataplmdiscuss["user"]=$_SESSION["name"];
			M("plmbackmoney")->add($dataplmdiscuss);
		}
		...
		if ($list !== false) { //保存成功
			...
			//$this->success('新增成功!');
			if($_REQUEST[operate_type]=="1")
			{
				$this->redirect('../Secondpublishcheck/index');
			}
			else
			{
				$this->redirect('../Backmoney/index');
			}
		} else {
			//失败提示
			$this->error('新增失败!');
	...
	function draftfirst() {
		...
		$vo[backmoneytotal] = M("plmbackmoney")->where("plmid=".$id)->sum("backmoney");
		$vo[backmoneytotal] = round($vo[backmoneytotal],2);
		...
		$plmbackmoney=M("plmbackmoney")->where("plmid=".$id)->select();
		$this->assign('plmbackmoney', $plmbackmoney);
	...
	public function foreverdelete() {
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
				...
				{
					$backmoneytotal = M("plmbackmoney")->where("plmid=".$info[plmid])->sum("backmoney");
                    $oldinfo=M("Project")->where("id=".$info[plmid])->find();
					$money=$oldinfo["audit"]-$backmoneytotal;
					M("Project")->where("id=".$info[plmid])->setField("money",$money);
