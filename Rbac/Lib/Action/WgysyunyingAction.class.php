<?php
class WgysyunyingAction extends CommonAction {			
	//过滤查询字段
	...
	public function index() {
		...
		$map[design_status]=array("in","联合验收通过,竣工待验收,项目待验收,验收审核退回,完成验收");//待施工,施工中,完成施工
		...
		if(1)//false!==strstr($_SESSION["role"],"经理")
		{
			//$this->assign("yunyingyanshou_noaccess","0");
		}
	...
	function insert() {
		...
		if(!empty($_FILES['file2']['name'][0]))
		{
			...
			$model->finishphoto=$newnameall;
			$model->finishphotofilename=$filenameall;
			if($_SESSION[app]=="1")
				$handlehistory.=$_SESSION['loginUserName']."于".$date."上传项目验收文件</br>------------------</br>";
			else
				$handlehistory.=$_SESSION['loginUserName']."于".$date."上传项目验收文件</br>------------------</br>"; 
		}
	...
	public function advancecomplete() {
        ...
        if (!empty($model)) {
            ...
            if (isset($id)) {
				...
				$model->where($condition)->setField("advance",date("Y-m-d",time()));
				
				
				$handlehistory.=$_SESSION['loginUserName']."于".$date."操作超前完成项目"."</br>------------------</br>";
	...
	function projectcomplete() {
		...
		$model->where($condition)->setField("design_status","竣工待验收");
		$model->where($condition)->setField("finish_time",time());
		
		$handlehistory.=$_SESSION['loginUserName']."于".$date."提交竣工验收"."</br>------------------</br>";
		$model->where($condition)->setField("handlehistory",$handlehistory);
	
		
		$this->success('提交竣工验收成功！');
		$this->forward();
	}
}
?>