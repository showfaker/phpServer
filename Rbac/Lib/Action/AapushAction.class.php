<?php
	...
	define('APPKEY','IVWVIMbwzG6gnRzEO0k8c9');
	define('APPID','eXvT8f7R3aA3JqlkqwEVx1');
	define('MASTERSECRET','PDN7zK6EZV6KUXsQ8HMxtA');
	define('CID','e10842ce6f85010873f8b86efce3e1f1');
	define('DEVICETOKEN','');
	define('Alias','请输入别名');
	...
	class AapushAction extends Action {
	
	
		function index()
		{
			$this->pushMessageToSingle("e10842ce6f85010873f8b86efce3e1f1","123",0,1);
		}
		...
		//单推接口案例
		function pushMessageToSingle($clientid,$message,$badge,$phonetype){
			//$igt = new IGeTui(HOST,APPKEY,MASTERSECRET);
		...
		//	$template = $this->IGtTransmissionTemplateDemo();
		    ...
		    if($phonetype==1)
		    {
		    	$template = $this->IGtTransmissionTemplateDemo($message,$badge);
		    }
		    if($phonetype==0)
		    {
		    	$template = $this->IGtNotificationTemplateDemo($message,$badge);
		    }
		...
		function IGtNotificationTemplateDemo($message,$badge){
			$template =  new IGtNotificationTemplate();
			$template->set_appId(APPID);//应用appid
			$template->set_appkey(APPKEY);//应用appkey
			$template->set_transmissionType(1);//透传消息类型
			$template->set_transmissionContent($message);//透传内容
			$template->set_title("英达热再生");//通知栏标题
		...
		function IGtTransmissionTemplateDemo($message,$badge){
		...
		//        $message = new IGtSingleMessage();
			...
			$alertmsg->actionLocKey="ActionLockey";
			$alertmsg->locKey=$message;
			$alertmsg->locArgs=array("locargs");
			$alertmsg->launchImage="launchimage";
		//        IOS8.2 支持
			$alertmsg->title="英达热再生项目汇总";
			$alertmsg->titleLocKey="TitleLocKey";
			$alertmsg->titleLocArgs=array("TitleLocArg");
			$apn->alertMsg=$alertmsg;
			$apn->badge=(int)($badge);
