...
class RoleModel extends CommonModel {
	public $_validate = array(
		...
		array('name','','职位已经存在',self::EXISTS_VAILIDATE,'unique',self::MODEL_INSERT),
		);

	public $_auto		=	array(
		array('level',2,self::MODEL_INSERT,'string'),
	...
	function delGroupModule($groupId,$appId)
	{
		...
		$result = $this->db->execute('delete from '.$table.' where level!=1 and pid!=0 and role_id='.$groupId);
		if($result===false) {
			return false;
		}else {
			return true;
	...
	function getGroupModuleList($groupId,$appId)
	{
		...
		$rs = $this->db->query('select b.id,b.title,b.name,a.type,a.approve from '.$table.' as a ,'.$this->tablePrefix.'node as b where a.node_id=b.id and  b.pid!=0 and a.role_id='.$groupId.' ');
		return $rs;
	}
