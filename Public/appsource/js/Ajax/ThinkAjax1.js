...
var ThinkAjax = {
	...
	ajaxResponse:function(request,target,response){
		...
		if (g(target))
		{
			...
			// 提示信息停留5秒
			if (this.showTip)
				this.intval = window.setTimeout(function(){fadeOut(target)},3000);
		}
	},
	// 发送Ajax请求
	...
	{
		var _self = this;
	    addEvent(source,event,function (){_self.send(url,vars,response,target,tips)});
	},
	// 页面加载完成后执行Ajax操作
	load:function(url,vars,response,target,tips)
	{
		var _self = this;
	    windowAddEvent('load',function (){_self.send(url,vars,response,target,tips)});
	},
	// 延时执行Ajax操作
