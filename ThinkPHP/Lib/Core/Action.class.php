...
abstract class Action {
   ...
   /**
    ...
    protected function error($message,$jumpUrl='',$ajax=false) {
		if($_SESSION[app]=="1")
		{
			$this->dispatchJumpm($message,0,$jumpUrl,$ajax);
		}
        else
		{
	...
	protected function errorm($message,$jumpUrl='',$ajax=false) {
        $this->dispatchJumpm($message,0,$jumpUrl,$ajax);
    }
    ...
    protected function success($message,$jumpUrl='',$ajax=false) {
		if($_SESSION["app"]=="1")
		{
			$this->dispatchJumpm($message,1,$jumpUrl,$ajax);
		}
		else
		{
	...
	protected function successm($message,$jumpUrl='',$ajax=false) {
        $this->dispatchJumpm($message,1,$jumpUrl,$ajax);
    }
	...
	private function dispatchJumpm($message,$status=1,$jumpUrl='',$ajax=false) {
        // 判断是否为AJAX返回
        if($ajax || $this->isAjax()) $this->ajaxReturn($ajax,$message,$status);
        if(!empty($jumpUrl)) $this->assign('jumpUrl',$jumpUrl);
        // 提示标题
        ...
        if($status) { //发送成功信息
            ...
            // 默认操作成功自动返回操作前页面
            if(!$this->view->get('jumpUrl')) $this->assign("jumpUrl",$_SERVER["HTTP_REFERER"]);
            $this->display(C('TMPL_ACTION_SUCCESSM'));
        }else{
            $this->assign('error',$message);// 提示信息
            //发生错误时候默认停留3秒
            if(!$this->view->get('waitSecond'))    $this->assign('waitSecond','3');
            // 默认发生错误的话自动返回上页
            if(!$this->view->get('jumpUrl')) $this->assign('jumpUrl',"javascript:history.back(-1);");
            $this->display(C('TMPL_ACTION_ERRORM'));
            // 中止执行  避免出错后继续执行
            exit ;
        }
    }
